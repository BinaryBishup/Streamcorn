...
/**
 * Fetch all seasons with episodes for a TV show from TMDB
 */
export async function fetchAllSeasonsWithEpisodes(tvId: number): Promise<Array<{
  season_number: number;
  name: string;
  overview: string;
  episode_count: number;
  ...
  try {
    // First fetch the TV show details to get season list
    const tvDetails: any = await fetchTVShowDetails(tvId);
    ...
    const seasonPromises = tvDetails.seasons.map(async (season: any) => {
      const response = await fetch(
        `${TMDB_BASE_URL}/tv/${tvId}/season/${season.season_number}?api_key=${TMDB_API_KEY}&language=en-US`,
        { next: { revalidate: 86400 } }
      );

      if (!response.ok) {
        console.error(`Failed to fetch season ${season.season_number}:`, response.statusText);
        return null;
      }
    ...
    return seasonsData.filter(s => s !== null);
  } catch (error) {
    console.error(`Error fetching all seasons for TV show ${tvId}:`, error);
    return null;
  }
}
